...
<div class="full_w">
	...
	?>
		...
		<div class="element">
			...
			</label> <?php echo $form->textField(Catagories::model(), 'name', array('size' => 50, 'maxlength' => 255,'value'=>$model->name)); ?>
		</div>
		<div class="element">
			<label for="name">Status Code <span class="red">(required)</span>
			</label><select name="Catagories[status]">
					<option value="">--Select--</option>
					<option value="A" <?php echo $model->status == 'A' ? 'selected="selected"' : ''?>>ACTIVE</option>
					<option value="I" <?php echo $model->status == 'I' ? 'selected="selected"' : ''?>>INACTIVE</option>
			</select>
		</div>
